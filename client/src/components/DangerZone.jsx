      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title="Delete Account">
        <div className="space-y-4">
          ...
          <input
            ...
            className="w-full px-3 py-2.5 bg-surface border border-border rounded-xl text-foreground focus:outline-none focus:ring-2 focus:ring-red-500"
          />
          <div className="flex justify-end gap-3">
            <button
              onClick={() => setOpen(false)}
              disabled={loading}
              className="px-4 py-2 text-muted hover:bg-surface-2 rounded-lg font-medium disabled:opacity-50"
            >
              Cancel
            </button>
            <button
